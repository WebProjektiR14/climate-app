...
export default class DataV1MonthlyChart extends React.Component {
  ...
  componentDidMount() {
    axios.get("http://localhost:8080/datav1gm").then((res) => {
      this.setState({ chart1Data: res.data });
    });

    axios.get("http://localhost:8080/datav1nm").then((res) => {
      this.setState({ chart2Data: res.data });
    });

    axios.get("http://localhost:8080/datav1sm").then((res) => {
